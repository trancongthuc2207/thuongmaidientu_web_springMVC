...
<c:forEach items="${productByID}" var="p" begin="0" end="2">
            ...
            <c:forEach items="${shop_product}" var="s_item">
                <c:if test="${s_item.product.idProduct == p.idProduct && s_item.idDiscount.idDiscount != '1'}">
                    <c:if test="${s_item.idDiscount.percentDiscount != null}">
                        <p class="badge rounded-pill bg-danger-10 text-danger custom-badge"
                           style="font-size: 30px">${s_item.idDiscount.nameDiscount}</p>
            ...
            <div class="row">
                <div class="col-sm-10 col-md-5 col-lg-5">
                    <sec:authorize access="hasAnyRole('ROLE_USER')">
                        <c:if test="${idOr != null}">
                            <a onclick="clickAddCartPro(${idOr},${p.idProduct},'${idCust}')"
                               href="javascript:void(0);"
                               class="btn btn-success btn-lg">Thêm vào giỏ hàng</a>
                        </c:if>
                    </sec:authorize>
                </div>
                <div class="col-sm-10 col-md-5 col-lg-5" style="display: flex">
    ...
    <sec:authorize access="hasAnyRole('ROLE_USER')">
        ...
        <div class="cmt" style="border: 1px solid black; padding: 20px; margin-top: 10px">
            ...
            <form class="cmt-cnt was-validated" style="display: flex;padding: 15px">
                <textarea style="width: 100%" type="text" id="content-cmt" required></textarea>
                <a onclick="clickAddComment()" class="btn btn-primary">Thêm</a>
            </form>
        </div>
    </sec:authorize>
    </div>
    ...
    <section class="section-b-space blog-detail-page review-page">
        <div class="container" style="margin-top: 0px">
            <div class="row">
                <div class="col-sm-12">
                    <h2>Bình luận:</h2>
                    <ul id="areaCmt" class="comment-section">
                        <c:if test="${list_cmt.size() > 0}">
                            <c:forEach items="${list_cmt}" var="lst_cmt">
                                <li style="width: 100%">
                                    <div class="media">
                                        <c:if test="${lst_cmt.idCusCmt.image.startsWith('https') == false}">
                                            <img src="https://bootdey.com/img/Content/avatar/avatar7.png" alt="Generic placeholder image">
                                        </c:if>
                                        ...
                                        <div class="media-body cmt-in4">
                                            <h6 class="name_cmt">${lst_cmt.idCusCmt.nameC} <span> ${lst_cmt.dateCreate} </span></h6>
                                            <p>${lst_cmt.content}</p>
                                            <ul class="comnt-sec">
                                                <li>
                                                    <a href="#">
...
<script>
    ...
    window.onload = function () {
        let dates = document.querySelectorAll(".cmt-in4 > .name_cmt > span");
    ...
    function clickSendRpProduct(idShop, idCus, idPro) {
        if (${canComment == true}) {
            ...
            alert('Bạn chưa từng mua sản phẩm này, nên không thể báo cáo');
        }
    }
    ...
    function clickAddComment() {
        if (${canComment == true}) {
            ...
            area.innerHTML = `<li style="width: 100%">
                                    <div class="media">
                                        <c:if test="${cus.image.startsWith('https') == false}">
                                            <img src="https://bootdey.com/img/Content/avatar/avatar7.png" alt="Generic placeholder image">
                                        </c:if>
                                        ...
                                        <div class="media-body cmt-in4">
                                            <h6 class="name_cmt">${cus.nameC} <span> ` + str_cmt + ` </span></h6>
                                            <p>` + ctn_cmt.value + `</p>
                                            <ul class="comnt-sec">
                                                <li>
                                                    <a href="#">
            ...
            alert('Bạn chưa từng mua sản phẩm này, nên không thể bình luận');
        }
    }
